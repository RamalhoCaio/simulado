"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
const SecretariaServices_1 = __importDefault(require("../services/SecretariaServices"));
class SecretariaController {
    constructor() { }
    listarSecretaria(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const secretarias = yield SecretariaServices_1.default.listarSecretaria();
            return res.status(200).json({
                status: 'ok',
                pacientes: secretarias,
            });
        });
    }
    atualizarSecretaria(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            return res.send('Atualizar Secretaria');
        });
    }
    criarSecretaria(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            const newSecretaria = req.body;
            const result = yield SecretariaServices_1.default.criarSecretaria(newSecretaria);
            if (result) {
                res.status(200).json({
                    status: 'tudo ok',
                    paciente: result,
                });
            }
            else {
                res.status(200).json({
                    status: 'erro',
                });
            }
        });
    }
    deletarSecretaria(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            return res.send('Deletar Paciente');
        });
    }
}
exports.default = new SecretariaController();
